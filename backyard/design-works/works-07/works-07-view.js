  ...
  for (let i = 0; i < contents.length; i++) {
    contents[i].addEventListener('click', () => {
      ...
      for (let i = 0; i < contentsObjArray.length; i++) {
        if (contentsObjArray[i].info.id == newFocusId) {
...
function showFocusedContent(focused) {
  ...
  // フォーカス中のhtml要素
  let focusedElement = document.getElementById(focus.info.id);
  focusedElement.classList.add('focus');

  // タイトルの表示
  showFocusedTitle(focus);
  // 画像の表示
  showFocusedImage(focus);
  // コメントの表示
  showFocusedComment(focus);

  // モーダルで開くか? ウィンドウを切り替えるか?
  let conetntImg = document.getElementById('content-img');
  conetntImg.classList.toggle('modal', (focused.info.modal == true));
...
// モーダルウィンドウを開く
function showModal() {
  console.log(document.getElementById('show-modal').checked);
  document.getElementById('modal-img').src = `../images/${focusObj.info.id}.png`;
}
...
function showFocusedComment(focused) {
  ...
  let genres = addCommentInfoMulti(focus, 'genre', 'comment-genre');

  let year = addCommentInfoSingle(focus, 'year', 'comment-year');

  if (focus.warn != '') {
    let warns = addCommentInfoMulti(focus, 'warn', 'comment-warn');
    focusedComment = `${genres} / ${warns}<br>${focus.info.comment}<br>${year}`;
  } else {
    focusedComment = `${genres}<br>${focus.info.comment}<br>${year}`;
  }
...
function addCommentInfoMulti(focused, txt, className) {
  ...
  let addedElem;
  let noHyphen;
  let n = focus[txt].length;
  for (let i = 0; i < n; i++) {
    noHyphen = focus[txt][i].replace(/-/g, ' ');
...
function addCommentInfoSingle(focused, txt, className) {
  let focus = focused;
  let resultText = `<span class="${className}">${focus[txt]}</span>`;
  return resultText;
}
...
function scrollToFocus() {
  let focusedElement = document.getElementById(focusObj.info.id);
