...
function newFunction(selectData, contentListData) {
  ...
  checkContentsClick(arrayGenre[0], arrayGenre);
}
...
document.getElementById('select-hide').addEventListener('change', function() {
...
function arrangeContentBox(focusObj) {
  ...
  if (document.getElementById('select-hide').checked) {
...
var contentsClicked = checkContentsClick(globalVar.glContentsObjArray);
...
let imageClicked = openWindow(globalVar.glFocusObj);
...
// 矢印キーでフォーカスを移動など
let keyDown = keyEvents(globalVar.glFocusObj);
...
function showFocusedImage(focusObj) {
  ...
  document.getElementById('content-img').src = `../images/${fcsObj.info.id}.png`;
