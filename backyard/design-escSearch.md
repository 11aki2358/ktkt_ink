...
* aリンクに<code class="language-html">rel="nofollow"</code>をつける(本サイト外のページに至るリンクには必須か) :  
  <a href="https://www.just-size.net/support/tips_searchdeny.php" target="_blank">link</a>  
...
* 一番安全な方法は、パスワード制限を付けるなどして、サーバ側で閲覧管理をすること(ロボットは絶対に侵入できない)。  
  ...htaccessなどを置ける環境であれば、サーバ側からの制限をかけることが出来る(Basic認証など)。  
  Vercelの場合、ディレクトリpublicにしかBasic認証を掛けることが出来ない
* プログラムによる制限:  
  phpなどを使うやりかた(フォーム認証)。  
  データベース不要で実装できることもある。
  Vercelの場合、phpやRubyを使えるのは、ディレクトリapi以下のファイルのみ。  
  javaScriptで同様のプログラムを書くことも不可能ではない(が、phpプログラムの実行は絶対であるのに対し、javascriptの実行は無効化できるので、む～～～ん...)