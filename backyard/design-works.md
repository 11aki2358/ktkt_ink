...
* WARNボタンの表示・非表示
* ヤバいサムネにぼかしを入れる
* ROUGHボタンの追加(スケッチとか練習とか)
* **サムネ画像は200px, モーダルではオリジナルサイズ、的なやつ**

だけれど、
* WARNタグが付けられるコンテンツを置くならばPHPのあれこれを完了しなければならない。今はやりたくない。
* コンテンツが少ないうちは、真面目に描いた絵もROUGHもごちゃまぜで十分かな...  
  
なかんじ。
* WARNボタンはとりあえずコメントアウト
* 今後のためにjsonファイルの検索用key値に<code class="lang-json">rough</code>を入れて置く
