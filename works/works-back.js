...
function onLoadAction() {
  ...
  document.documentElement.style.setProperty('--type1-main-color', `#5cc5ef`);

  if (sessionStorage.getItem('firstVisit-work-back') != '1') {
    // このセッションで初めてworksを訪れた場合は...
    selectContent();
    sessionStorage.setItem('firstVisit-workk-back', '1');
  } else {
...
function selectContent() {
  ...
  sessionStorage.setItem('selectButtonData-back', selectDataStr);

  // jsonファイルから、コンテンツのリストを引っ張ってくる
  // ローカルの場合、LiveServer起動しないとエラーになる
  fetch('./works-back.json')
    .then(response => {
...
function newFunction(selectData, contentListData) {
  ...
  sessionStorage.setItem('contentsList-back', genreJson);
  ...
  sessionStorage.setItem('focusContent-back', JSON.stringify(arrayGenre[0]));
...
function showContentListOnLoad() {
  let sessionJson = sessionStorage.getItem('contentsList-back');
  globalVar.glContentsObjArray = JSON.parse(sessionJson);
  showContentList(globalVar.glContentsObjArray);

  sessionFocusObj = sessionStorage.getItem('focusContent-back');
...
function moveFocus(focusNum, contentsArray) {
  ...
  sessionStorage.setItem('focusContent-back', JSON.stringify(globalVar.glFocusObj));
...
function setCheckLabel() {
  const selectDataStr = sessionStorage.getItem('selectButtonData-back');
